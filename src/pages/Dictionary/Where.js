...
import styles from './Where.less';
import dictionariesMgr from '@/services/dictionariesMgr';

const Option = Select.Option;
const TabPane = Tabs.TabPane;

class Where extends PureComponent {
  state = {
    isLoading: false,
    selectedRows: [],
    selectedRowsLevel1: [],
  ...
  componentDidMount = async () => {
    this.setIsLoad(true);
    const dictionary = await dictionariesMgr.getWheres();
    ...
    const whereRoot = dictionariesMgr.dictionaries.find(
      ele => ele.itemParentId === null && ele.rootIndex === -13
    );
    this.setState({ level1Root: whereRoot });
