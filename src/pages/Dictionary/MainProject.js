...
import styles from './MainProject.less';
...
class PersonalProp extends PureComponent {
  ...
  componentDidMount = async () => {
    this.setIsLoad(true);
    const dictionary = await dictionariesMgr.getMainProjects();
    ...
    const mainProjectRoot = dictionariesMgr.dictionaries.find(
      ele => ele.itemParentId === null && ele.rootIndex === -7
    );
    this.setState({ level1Root: mainProjectRoot });
        ...
        <Modal
          ...
          onOk={async () => {
            if (
              ...
              newLevel1Item.itemSecondName !== null
            ) {
