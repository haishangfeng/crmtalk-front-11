...
class BillDictionary extends PureComponent {
  state = {
    isLoading: false,
    selectedRows: [],
    selectedRowsLevel1: [],
  ...
  componentDidMount = async () => {
    this.setIsLoad(true);
    const dictionary = await dictionariesMgr.getBills();
    const filters = [];
  ...
  handleOk = async () => {
    ...
    if (
      newItemName.trim() === '' ||
      newItemParentName.trim() === '' ||
      newItemSecondName.trim() === ''
    ) {
