...
class Consultation extends PureComponent {
  ...
  componentDidMount = async () => {
    this.setIsLoad(true);
    const dictionary = await dictionariesMgr.getMainFrom();
  ...
  handleModalInputPs = value => {
    this.setState({ newItemPs: value });
  };
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: formatMessage({ id: 'dictionary.table.s7' }),
        dataIndex: 'ps',
        width: 200,
      },
      {
      ...
      <Card>
        ...
        >
          <Form layout="inline">
            ...
            <Form.Item label="前缀(用于生成预约号)">
              <Input
                value={newItemPs}
                placeholder="W, M, A, ..."
                onChange={e => this.handleModalInputPs(e.target.value)}
              />
            </Form.Item>
        ...
        >
          ...
          <h4>前缀（用于生成预约号）：</h4>
          <Input placeholder={newItemPs} onChange={e => this.handleModalInputPs(e.target.value)} />
          <h4>字典项名称：</h4>
          <Input
